...
const ProductInfo = () => {
  ...
  const addToFav = async () => {
    if (!user.loggedIn) {
      ...
      try {
        setLoading(true);
        const response = await Request('POST', `/prod/addfav/`, { productId: product.id ,variantId:variant.id});
        setLoading(false);
  ...
  const addToCart = async () => {
    if (!user.loggedIn) {
      ...
      try {
        const response = await Request('POST', `/cart/add/${product.id}`, { quantity ,variantId:variant.id});
        if (response.status === 200) {
          navigate('/cart');
        }
      } catch (error) {
  ...
  const getValidColor = async (colorName) => {
    try {
      const response = await fetch(`https://www.thecolorapi.com/id?name=${colorName}`);
  ...
  const handleColorUpdate = async (selcolor) => {
    ...
    setColor(selcolor);  // Update the selected color state
        ...
        const filteredVariants =  variants.filter(variant => 
            variant.color === selcolor && variant.size === selectedsize
        );
...
const handleSizeUpdate = (selsize) => {
    setSize(selsize);    // Update the selected size state
      ...
      console.log(selsize)
        const filteredVariants = variants.filter(variant => 
            variant.color === selectedcolor && variant.size === selsize
        );
                ...
                <div className="mt-3 flex select-none flex-wrap items-center gap-2">
                  {sizes.map((sizeOption) => (
                    <label key={sizeOption}>
                      ...
                      />
                      <p className={`peer-checked:bg-black peer-checked:text-white rounded-lg border border-black px-6 py-2 font-bold ${selectedsize === sizeOption ? 'bg-black text-white' : ''}`}>
                        {sizeOption}
                      </p>
                    </label>
                  ))}
  ...
  <button
    key={colorName}
    className={`w-10 h-10 rounded-full   ${variant.color === colorName ? 'scale-125 border-2 border-black' : 'border-2 border-black'}`}
    style={{ backgroundColor: colorName }} // Directly using color name for background
    onClick={() => handleColorUpdate(colorName)} // Set the selected color
  >
    <span className="sr-only">{colorName}</span>
                ...
                <div className="mt-10 flex flex-col items-center justify-between space-y-4 border-t border-b py-4 sm:flex-row sm:space-y-0">
                  <div className="flex items-end">
                    <h1 className="text-3xl font-bold">${variant && variant.retail_price ? variant.retail_price : ''}</h1>
                  ...
                  <div className="flex space-x-4">
                    ...
                    <button
                      ...
                      onClick={() => {
                        navigate('/checkout', {
                          state: { fromCart: false, productId: product.id, quantity,variantId:variant.id },
                        });
                      }}
